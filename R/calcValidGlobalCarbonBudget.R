...
calcValidGlobalCarbonBudget <- function(cumulative = FALSE) {
  ...
  peat <- magclass::collapseNames(allOut[, , peatVar])
  # per-model peat child (matches MAgPIE +|Peatland) and Excl Peatland (net - peat)
  peatChild <- NULL
  excl <- NULL
  for (m in bkModels) {
    luc <- allOut[, , paste0(m, ".", lucVar)]
    pt  <- luc
    pt[, , ] <- peat[, , ]
    ex  <- luc
    ex[, , ] <- luc[, , ] - peat[, , ]
    peatChild <- magclass::mbind(peatChild, magclass::setNames(pt, paste0(m, ".", peatVar)))
    excl      <- magclass::mbind(excl,      magclass::setNames(ex, paste0(m, ".", exclVar)))
  }
  allOut <- allOut[, , paste0("GCB.", peatVar), invert = TRUE]   # drop GCB-only peat memo; re-add for all models
